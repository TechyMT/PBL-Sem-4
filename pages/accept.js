...
export default function App() {
  ...
  useEffect(() => {
    ...
    return () => {
      inputRefs.forEach((ref, index) => {
        ref.current && ref.current.removeEventListener('input', (event) => {
          handleInput(event, index);
        });
      });
    };
  ...
  return (<div>
    <h1>PRINT THOSE SHITS</h1>
    <div className={styles.body}>
      <form className={styles.form} onSubmit={handleClick}>
        <div className={styles.inputContainer}>
      <input  className={styles.input} type="text" id="input1" name="input1" maxLength="1" ref={inputRefs[0]} />

      <input className={styles.input} type="text" id="input2" name="input2" maxLength="1" ref={inputRefs[1]} />

      <input className={styles.input} type="text" id="input3" name="input3" maxLength="1" ref={inputRefs[2]} />

      <input className={styles.input} type="text" id="input4" name="input4" maxLength="1" ref={inputRefs[3]} />

      <input className={styles.input} type="text" id="input5" name="input5" maxLength="1" ref={inputRefs[4]} />
      </div>
    ...
    <button type="submit">Verify</button>
     ...
     <p>{inputValue}</p>
      ...
      <div className={styles.inputContainer}>     <ul>
        {result.length>0?(
          <div>
